...
class RedisHelper(ISqlHelper):
    ...
    def get_keys(self, conditions):
        select_keys = {
            self.get_index_name(key, conditions[key])
            for key in conditions.keys()
            if key in self.index_names
        }
        if "ip" in conditions and "port" in conditions:
            return self.redis.keys(
                self.get_proxy_name(conditions["ip"], conditions["port"], "*")
            )
        if select_keys:
    ...
    def select(self, count=None, conditions=None):
        ...
        querys = (
            {k: v for k, v in conditions.items() if k in self.index_names}
            if conditions
            else None
        )
        if querys:
            ...
            objects = list(
                self.redis.zrevrangebyscore(
                    self.get_index_name("score"), "+inf", "-inf", start=0, num=count
                )
            )
